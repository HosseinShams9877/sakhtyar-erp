...
/* ═══════════════════════════════════════════════════════
   ...
   function useDashboardData() {
    const { activeProject } = useProject(); // 👈 اضافه کن
    ...
    useEffect(() => {
      ...
      loadData();
    }, [activeProject?.id]); // 👈 وابستگی به activeProject
          ...
          {showQuickForm && (
            <div className="fixed inset-0 z-50 bg-black/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
              <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-800">
                <div className="bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-500 dark:to-blue-600 px-5 py-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-lg font-bold text-white">ثبت فاکتور سریع</h3>
                    <button
                  ...
                  <p className="text-blue-100 text-xs mt-1">اطلاعات فاکتور را وارد کنید</p>
                </div>
    ...
    <p className="text-xs text-amber-600 mt-1">
      هیچ تامین‌کننده‌ای به این پروژه متصل نیست
                  ...
                  <button
                    ...
                    disabled={submitting}
                    className="w-full bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-500 dark:to-blue-600 text-white py-3.5 rounded-xl font-bold mt-4 hover:from-blue-700 hover:to-blue-800 dark:hover:from-blue-600 dark:hover:to-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                  >
                    {submitting ? (
