...
<div className="mt-4">
  ...
  <div className="space-y-4">
    {items.map((row, idx) => (
      <div
        key={row._key}
        className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm space-y-3"
      >
        {/* انتخاب کالا - ساده و بدون Popover */}
        <div>
          <div className="text-xs text-gray-500 mb-1">نام کالا</div>
          <select
            ...
            onChange={(e) => {
              ...
              if (selectedMaterial) {
                selectMaterial(row._key, selectedMaterial);
              }
            }}
            className="w-full h-11 px-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all"
          >
            <option value="">انتخاب کالا...</option>
            {materials.map((material) => (
              <option key={material.id} value={material.id}>
                {material.name} ({UNIT_LABELS?.[material.unit] || material.unit})
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <div className="text-xs text-gray-500 mb-1">واحد</div>
            <select
              value={row.unit}
              onChange={(e) => updateItem(row._key, 'unit', e.target.value)}
              className="w-full h-11 px-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm"
            >
          ...
          <div>
            ...
            <input
              ...
              className="w-full h-11 px-3 text-center rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm"
              dir="ltr"
              placeholder="0"
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <div className="text-xs text-gray-500 mb-1">قیمت واحد (ریال)</div>
            <input
              type="number"
              value={row.unitPrice || ''}
          ...
          <div>
            <div className="text-xs text-gray-500 mb-1">مبلغ (ریال)</div>
        ...
        <button
          ...
          className="w-full py-2.5 rounded-xl border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-sm font-medium flex items-center justify-center gap-2 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all"
        >
    ...
    <button
      type="button"
      onClick={addItemRow}
      className="w-full py-3 rounded-xl border-2 border-dashed border-emerald-300 dark:border-emerald-700 text-emerald-600 dark:text-emerald-400 text-sm font-bold flex items-center justify-center gap-2 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 transition-all"
    >
    ...
    <div className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/30 dark:to-teal-950/30 rounded-2xl p-4 flex items-center justify-between border border-emerald-200 dark:border-emerald-800">
      ...
      <span className="text-lg font-extrabold text-emerald-700 dark:text-emerald-400">{formatCurrency(itemsTotal)}</span>
    </div>
  </div>
</div>
